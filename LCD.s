...
loopcommand1
	...
	AND		R2, #0x10			; read bit 4
	ADDS	R2, #0				; set conditional bits
	BNE		loopcommand1		; keep reading until bit 4 is low
	LDR		R1, =GPIO_PORTA_DATA_R
	LDR		R2, [R1]			; read PA
	BIC		R2, #0x40			; clear PA6
	STR		R2, [R1]			; write changes
	LDR		R1, =SSI0_DR_R
	STRB	R0, [R1]			; write command to SSI0_DR_R
	LDR		R1, =SSI0_SR_R
loopcommand2
	LDR		R2, [R1]			; read SSI0_SR_R
	AND		R2, #0x10			; read bit 4
	ADDS	R2, #0				; set conditional bits
	BNE		loopcommand2		; keep reading until bit 4 is low
    BX 		LR                  ;   return
...
loopdata1
	LDR		R2, [R1]			; read SSI0_SR_R
	AND		R2, #0x02			; read bit 1
	ADDS	R2, #0				; set conditional bits
	BEQ		loopdata1			; loop until bit 1 is high
	LDR		R1, =GPIO_PORTA_DATA_R
	LDR		R2, [R1]			; read PA
	ORR		R2, #0x40			; set PA6
	STR		R2, [R1]			; write changes
	LDR		R1, =SSI0_DR_R
	STRB	R0, [R1]			; write data to SSI0_DR_R
	BX  	LR                  ;   return
